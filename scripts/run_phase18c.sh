#!/usr/bin/env bash
# Phase 18C: EEG → spatial_768x4x4 code with probe-on
#
# Steps:
#   1. Extract 4×4 bottleneck codes from existing spatial_768x4x4 checkpoint
#   2. Train CommonProbeModel on mean_pool(4×4 code) → [768] → normalize
#   3. Train EEG encoder: spatial_cosine loss + probe auxiliary (weight=0.01, start_epoch=5)
#   4. Evaluate: EEG → code → expand → RAE decode → visual grid
...
export PYTHONUNBUFFERED=1   # flush print() immediately — no 8KB block buffer
...
SLUG="phase18c_eeg_to_rae_4x4_probe"

# Locate VLM attributes — prefer runs01_40 version
...
elif [ -f "data/processed/clip_embeddings/vlm_attributes.json" ]; then
    VLM_ATTRIBUTES="data/processed/clip_embeddings/vlm_attributes.json"
else
...
# ============================================================
# Step 1: Extract 4×4 bottleneck codes
# ============================================================
echo ""
...
# ============================================================
# Step 2: Train probe on mean_pool(4×4 code) → [768]
# ============================================================
echo ""
...
# ============================================================
# Step 3: Train EEG encoder — spatial_cosine + probe
# ============================================================
echo ""
echo "=== Step 3: Training EEG encoder — Phase 18C ==="
...
echo ""
echo "=== Step 4: Evaluating Phase 18C ==="
python3 scripts/evaluate_rae_spatial.py \
    ...
    --num-samples 200 \
    --output-dir outputs/phase18c_rae_spatial_eval \
    --device cuda

echo ""
echo "=== Phase 18C complete! ==="
echo "  Run dir: $RUN_DIR"
echo "  Metrics: outputs/phase18c_rae_spatial_eval/generation_metrics.json"
echo "  Grid:    outputs/phase18c_rae_spatial_eval/generation_grid.png"
