#!/usr/bin/env bash
# Phase 18C-v2: EEG → spatial_768x4x4 code, probe-on, warm-started from Phase 16c
#
# Key differences from 18C:
#   - init_from: Phase 16c best.pt (temporal_attn_small + subject FiLM adapters)
#   - Output head is NOT loaded (size mismatch → init_skip_heads=True soft-load)
#   - Probe accuracy columns now logged correctly (bug fixed in train_eeg_clip.py)
#
# Run on RunPod: bash scripts/run_phase18c_v2.sh
...
SLUG="phase18c_v2_eeg_to_rae_4x4_probe_warmstart"
...
# ============================================================
# Step 3: Train EEG — warm-started from Phase 16c
# ============================================================
echo ""
echo "=== Step 3: Training EEG encoder — Phase 18C-v2 (warm-start from 16c) ==="
...
echo ""
echo "=== Step 4: Evaluating Phase 18C-v2 ==="
python3 scripts/evaluate_rae_spatial.py \
    ...
    --num-samples 200 \
    --output-dir outputs/phase18c_v2_rae_spatial_eval \
    --device cuda

echo ""
echo "=== Phase 18C-v2 complete! ==="
echo "  Run dir: $RUN_DIR"
echo "  Metrics: outputs/phase18c_v2_rae_spatial_eval/generation_metrics.json"
echo "  Grid:    outputs/phase18c_v2_rae_spatial_eval/generation_grid.png"
